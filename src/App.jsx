...
const brand = {
  ...
  payers: "Medicaid • Private Pay",
  entity: "Home Health Care LLC",
  palette: { sage: "#9FB6A2", deepSage: "#3F5D51", cream: "#F2EFE7", ivory: "#FAF7F1", lavender: "#CDBBD8", gold: "#D4AF37", text: "#22322B" },
};
...
// ----------------------
const Logo = ({ size = 36 }) => (
  <svg width={size} height={size} viewBox="0 0 64 64" fill="none" className="shrink-0" aria-label="Mindful Living logo">
    <circle cx="32" cy="32" r="31" stroke={brand.palette.gold} strokeWidth="2.5" fill={brand.palette.cream} />
    <path d="M32 18c6 8 2 12-1 15-3 3-6 6-6 10 0 4 3 7 7 7s7-3 7-7c0-4-3-7-6-10-3-3-7-7-1-15Z" fill={brand.palette.deepSage} opacity=".9" />
    <path d="M18 36c2 3 5 5 9 5M46 36c-2 3-5 5-9 5" stroke={brand.palette.gold} strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);
...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-emerald-900/10 backdrop-blur supports-[backdrop-filter]:bg-white/30" style={{ background: `${brand.palette.sage}99` }}>
      <Container className="flex items-center justify-between py-3">
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <CTAButton icon={Phone} to="/contact">{t("common.call")} {brand.phone}</CTAButton>
        </div>
      </Container>
    </header>
  );
...
const Footer = () => {
  ...
  return (
    <footer className="mt-20 border-t border-emerald-900/10 bg-white/70">
      <Container className="grid gap-10 py-10 md:grid-cols-4">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <Logo />
            <div>
              <div className="font-semibold text-emerald-900">{brand.name}</div>
              <div className="text-xs text-emerald-900/70">{t("hero.entity")}</div>
            </div>
          </div>
          <p className="text-sm text-emerald-950/80 max-w-xs">{brand.tagline}</p>
          <Badge>{t("hero.payers")}</Badge>
        </div>
        <div>
          <div className="mb-2 font-semibold text-emerald-900">{t("footer.explore")}</div>
          <ul className="space-y-2 text-sm">
            {[
              ...
              [t("policy.privacy"), "/privacy"],
              [t("policy.terms"), "/terms"],
            ].map(([label,to]) => (
              <li key={label}><Link className="text-emerald-900/80 hover:text-emerald-900" to={to}>{label}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          <div className="mb-2 font-semibold text-emerald-900">{t("footer.contact")}</div>
          <ul className="space-y-2 text-sm text-emerald-900/80">
            <li className="flex items-center gap-2"><Phone size={16}/> {brand.phone}</li>
            <li className="flex items-center gap-2"><Mail size={16}/> hello@mindfulliving.care</li>
            <li className="flex items-center gap-2"><MapPin size={16}/> Serving Greater Community</li>
            <li className="flex items-center gap-2"><Clock size={16}/> 24/7</li>
          </ul>
        </div>
        <div className="text-sm text-emerald-900/70">
          <p className="mb-3">{i18n.en.footer.rights(new Date().getFullYear(), brand.name)}</p>
          <p>{t("footer.demo")}</p>
        </div>
      </Container>
    </footer>
...
// Build services + values using i18n
  ...
  return (
    <Container className="py-16">
      ...
      <div className="mt-8 flex items-center justify-center gap-3">
        <CTAButton to="/contact" icon={Calendar}>{t("common.bookConsult")}</CTAButton>
        <OutlineButton to="/intake">{t("common.startIntake")}</OutlineButton>
      </div>
    </Container>
  );
};
...
const Hero = () => {
  const { t } = useT();
  return (
    <div className="relative overflow-hidden border-b border-yellow-700/10" style={{ background: `linear-gradient(135deg, ${brand.palette.cream}, ${brand.palette.lavender}40)` }}>
      <Container className="grid items-center gap-10 py-16 md:grid-cols-2 md:py-24">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <CTAButton>{t("common.call")} {brand.phone}</CTAButton>
    ...
    <main>
      ...
      <Container className="py-16">
        <div className="grid items-center gap-8 md:grid-cols-2">
          <div>
            <h3 className="text-2xl font-semibold text-emerald-950">{t("common.careThatCenters")}</h3>
            <p className="mt-2 text-emerald-900/80">{t("common.careThatCentersBlurb")}</p>
            <div className="mt-6 flex gap-3">
              <CTAButton to="/contact">{t("common.call")} {brand.phone}</CTAButton>
              <OutlineButton to="/about">{t("nav.about")}</OutlineButton>
            </div>
          </div>
...
const ServicesPage = () => {
  ...
  return (
    <main>
      ...
      <ServicesGrid />
      <Container className="py-8"><div className="rounded-2xl border border-yellow-500/30 bg-yellow-50 p-6 text-yellow-900"><p className="font-medium">{t("common.coverage")}</p><p className="text-sm opacity-90">{t("common.coverageNote")}</p></div></Container>
    </main>
  );
};
